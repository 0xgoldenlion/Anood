...
export function setTitle(product, price) {
	if (get(currentPage) == 'trade') {
		document.title = `${product} ${price} | Anood`;
	}
}
...
export function loadRoute(path, isInitial) {
	if (!path || path == '/' || path.includes('/home')) {
		...
		document.title = `Free Decentralized Perpetual Exchange | Anood`;
	} else if (path.includes('/trade')) {
		component.set(Trade);
		currentPage.set('trade');
		document.title = `Trade | Anood`;
	} else if (path.includes('/pool')) {
		component.set(Pool);
		currentPage.set('pool');
		document.title = `Pool | Anood`;
	} else if (path.includes('/buy')) {
		component.set(Buy);
		currentPage.set('buy');
		document.title = `Buy AND | Anood`;
	}
	hydrateData();
}
