<script>
  ...
  async function getSizeInUSD(_currencyLabel, _prices, _size) {
    ...
    if (_currencyLabel == "weth") {
      sizeInUSD = _prices["FIL-USD"] * _size;
...
<div class="order" class:loading={$isSubmittingLong || $isSubmittingShort}>
  ...
  <div class="buttons">
    {#if $currencyLabel != "weth" && $allowances[$currencyLabel] && $allowances[$currencyLabel]["trading"] * 1 == 0}
      <Button
        label={`Approve ${formatCurrency($currencyLabel)}`}
        onClick={() => {
          _approveCurrency();
        }}
      />
    {:else}
      <Button
        isRed={true}
        isLoading={$isSubmittingShort}
        label="Short"
        onClick={() => {
          _submitNewPosition(false);
        }}
      />
      <Button
        isLoading={$isSubmittingLong}
        label="Long"
        onClick={() => {
          _submitNewPosition(true);
        }}
      />
    {/if}
  </div>
  ...
  {#if !$address}
    <div class="note">
      Anood is an open protocol to trade crypto perpetuals with low fees. <a
        data-intercept="true"
        on:click={() => {
          showModal("Connect");
        }}>Connect your wallet</a
      > on Hyperspace to get started.
    </div>
