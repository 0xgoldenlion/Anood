<script>
  ...
  let poolEntries = [];
  $: poolEntries = Object.entries($pools).sort((a, b) => {
    return a[0] > b[0] ? -1 : 1;
  });

  let oldPoolEntries = [];
  $: oldPoolEntries = Object.entries($oldPools).sort((a, b) => {
    return a[0] > b[0] ? -1 : 1;
  });
...
<div class="pools">
    ...
    <div class="pool" class:loading={poolIsLoading[_currencyLabel] || !poolInfo.tvl}>
      ...
      <div class="stats">
        <div class="row">
          ...
          <div class="value">
            {getTVL(_currencyLabel, poolInfo, $address)}
            {formatCurrency(_currencyLabel)}
            {#if _currencyLabel == "weth" && poolInfo && poolInfo.tvl && $prices["ETH-USD"]}
      ...
      <div class="stats my-stats">
        <div class="row">
          <div class="label">
            ...
            {#if $allowances[_currencyLabel] && $allowances[_currencyLabel]["pool" + _currencyLabel] * 1 == 0}
              <a
                on:click={() => {
                  _approveCurrency(_currencyLabel);
                }}>Approve {formatCurrency(_currencyLabel)}</a
              >
            {:else}
              <a
                data-intercept="true"
                class:disabled={!poolInfo.tvl}
                on:click={() => {
                  showModal("PoolDeposit", { currencyLabel: _currencyLabel });
                }}>Deposit</a
              ><span class="sep">|</span><a
                class:disabled={poolInfo.userBalance == 0}
                data-intercept="true"
                on:click={() => {
                  showModal("PoolWithdraw", { currencyLabel: _currencyLabel, withdrawFee: poolInfo.withdrawFee, userBalance: poolInfo.userBalance });
                }}>Withdraw</a
              >
            {/if}
          </div>
        ...
        <div class="row">
          <div class="label">
            ...
            <a
              class:disabled={poolInfo.claimableReward == 0}
              on:click={() => {
                collectPoolReward(_currencyLabel);
              }}>Collect</a
            >
          </div>
          <div class="value">
            {formatToDisplay(poolInfo.claimableReward) || 0}
            {formatCurrency(_currencyLabel)}
  ...
  <!-- <div class="pool cap-pool" class:loading={poolIsLoading["cap"] || !$capPool.supply}>
    ...
    <div class="description">
      Stake AND to receive a share of revenue. <a href="#/buy">Buy AND</a>
    </div>
    ...
    <div class="stats my-stats">
        ...
        <div class="label">
          ...
          {:else}
            <a
              data-intercept="true"
              on:click={() => {
                showModal("PoolDeposit", { currencyLabel: "AND" });
              }}>Deposit</a
            ><span class="sep">|</span><a
              class:disabled={$capPool.userBalance == 0}
              data-intercept="true"
              on:click={() => {
                showModal("PoolWithdraw", { currencyLabel: "AND", userBalance: $capPool.userBalance });
              }}>Withdraw</a
            >
          {/if}
        </div>
        <div class="value">
          {formatToDisplay($capPool.userBalance)} AND <span class="grayed">({formatToDisplay($capPool.supply * 1 == 0 ? 0 : (100 * $capPool.userBalance) / $capPool.supply)}%)</span>
        </div>
      </div>
      ...
      {#each Object.entries($capPool.claimableRewards || {}) as [_currencyLabel, reward]}
        <div class="row">
          <div class="label">
            <div class="top-label">My {formatCurrency(_currencyLabel)} Rewards <!-- (<strong>{formatToDisplay($capPool.poolShares[_currencyLabel])}%</strong> of fees) -->
</div>
<!-- <a
              class:disabled={reward == 0}
              on:click={() => {
                collectCAPReward(_currencyLabel);
              }}>Collect</a
            >
          </div>
          <div class="value">
